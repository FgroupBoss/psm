...
一期核心微服务与试点前端已落地；模块划分基于 `design/01_一期落地设计`：
...
code
  backend/          后端微服务
  frontend/         前端应用（admin-web / mobile-site / dashboard-web 试点）
...
后端按业务域拆分微服务，服务间通过 API、消息和事件解耦。各服务含 `pom.xml`、源码、配置与测试；状态以 `design/01_一期落地设计/00_开发总览/项目进展总览.md` 为准。
...
| 服务 | 端口 | 职责 |
| --- | --- | --- |
| `psm-dual-prevention-service` | 18101 | 双重预防、隐患治理 |
| `psm-inspection-service` | 18102 | 智能巡检计划与执行 |
| `psm-location-service` | 18103 | 人员定位、围栏、封闭化 |
| `psm-video-service` | 18104 | 视频 AI 事件、过程监护 |
| `psm-integration-service` | 18091 | 监管园区映射、抽取与上报（M06） |
| `psm-report-service` | 18090 | 一期报表 + `/api/reports/phase2/summary` 二期聚合 |

进展见 [二期项目进展总览](../design/02_二期落地设计/00_开发总览/项目进展总览.md)。
